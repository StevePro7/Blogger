define(['jquery','domReady','jquery/ui','mage/cookies'],function($,domReady){'use strict';$.fn.comments=function(){var elements=[];(function lookup(element){$(element).contents().each(function(index,el){var elm=String(el).split('HTMLIFrameElement');if(elm.length>1){return;}
switch(el.nodeType){case 1:lookup(el);break;case 8:elements.push(el);break;case 9:var hostName=window.location.hostname,iFrameHostName=$('<a>').prop('href',element.prop('src')).prop('hostname');if(hostName===iFrameHostName){lookup($(el).find('body'));}
